...
unless(defined $nowrite) {
	...
	if($base=~/debian/i or $base=~/ubuntu/i) { todockerfile($fh, 'RUN apt-get update && apt-get -y install automake build-essential vim-tiny thc-ipv6 && ln -s /etc/alternatives/vi /usr/bin/vim'); }
